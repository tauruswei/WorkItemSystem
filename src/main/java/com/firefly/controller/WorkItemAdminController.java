...
import com.firefly.config.WebConfig;
...
public class WorkItemAdminController {
...
//	}
	...
	public String adminIndex(HttpServletRequest request, Model model,Principal principal) {
...
//		KeycloakPrincipal<KeycloakSecurityContext> keycloak = 
//		//1.接收参数
...
//			rb.setData(id);
////			这里是防止用户在key咩有过期的时候，再重新登录
//			if(!StrUtil.isBlank(strRedis.opsForValue().get(username))){
//				strRedis.delete(strRedis.opsForValue().get(username));
//			}
//			strRedis.opsForValue().set(username, id,webConfig.getSessionTimeOut(),TimeUnit.HOURS);
//			strRedis.opsForValue().set(id, username,webConfig.getSessionTimeOut(),TimeUnit.HOURS);
//			model.addAttribute("username", username);
//			model.addAttribute("token", rb.getData());
//			System.out.println(principal.getName());
...
//		}
	...
	public String updateWorkItemDetail(HttpServletRequest request) throws Exception {
		...
		strRedis.opsForValue().set(workItemDetail.getQuestionid(), sid.nextShort(),webConfig.getQuestionTimeOut(), TimeUnit.HOURS);
...
//		return "redirect:http://localhost:12306/firefly/queryWorkItemById?questionid="+request.getParameter("questionid");
		return "redirect:"+webConfig.getHttpPrefix()+"/firefly/queryWorkItemById?questionid="+request.getParameter("questionid");
	}
	...
	public String closeWorkItem(@RequestParam(value = "questionId") String questionid) throws Exception {
		...
		workItemDetail.setUpdatedtime(time);
		workItemDetailService.saveWorkItem(workItemDetail);
		return "redirect:"+webConfig.getHttpPrefix()+"/firefly/unsolvedQuestions?status=open";
	}
...
//	}
	...
	public String logout(HttpServletRequest request) throws ServletException {
		request.logout();
		return "redirect:"+webConfig.getHttpPrefix()+"/firefly/login";
//		return null;
	}
}
